...
class RecordingService {
        ...
        try {
            ...
            const request = {
                config: {
                    ...
                    model: 'default', // Use default model for faster response
                    useEnhanced: false, // Disable enhanced model for speed
                    enableWordTimeOffsets: false,
                    enableAutomaticPunctuation: false,
                    // Simpler speech adaptation for speed
                    speechContexts: [{
            ...
            console.log('Speech API response:', JSON.stringify(data, null, 2));
  ...
  private getAlternativeLanguageCodes(primaryCode: string): string[] {
      // Provide alternative dialects for better recognition
      const alternatives: Record<string, string[]> = {
          'es-ES': ['es-MX', 'es-US', 'es-419'],
      ...
      return alternatives[primaryCode] || [];
  }
